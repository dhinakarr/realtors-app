...
public class UserPerformanceRepository {
	...
	public List<SiteVisitDetailDTO> fetchSiteVisits(UUID userId, LocalDate fromDate, LocalDate toDate) {
		...
		return jdbcTemplate.query(sql, new Object[] { userId, fromDate, fromDate, toDate, toDate }, new RowMapper<>() {
			@Override
		...
		return jdbcTemplate.query(sql, new Object[] { userId, fromDate, fromDate, toDate, toDate }, new RowMapper<>() {
			...
			public SaleDetailDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
				...
				dto.setConfirmedAt(rs.getTimestamp("confirmed_at") != null
						? rs.getTimestamp("confirmed_at").toLocalDateTime().toLocalDate()
						: null);
				return dto;
			}
		});
	...
	public List<SaleDetailDTO> fetchSales(UUID userId, LocalDate fromDate, LocalDate toDate, boolean isCommon) {
		...
		// ✅ Apply filter ONLY for non-common users
		if (!isCommon) {
			sql.append(" AND s.sold_by = ? ");
			params.add(userId);
		}
		if (fromDate != null) {
			sql.append(" AND s.confirmed_at >= ? ");
			params.add(fromDate);
		}
		if (toDate != null) {
			sql.append(" AND s.confirmed_at <= ? ");
		...
		sql.append(" ORDER BY s.confirmed_at DESC ");
	...
	public List<ReceivableDetailDTO> fetchReceivable(UUID userId, LocalDate fromDate, LocalDate toDate,
		...
		StringBuilder sql = new StringBuilder("""
				SELECT p.payment_id, proj.project_name, pl.plot_number, c.customer_name,
				s.total_price, s.base_price,
...
// ✅ Apply filter ONLY for non-common users
		...
		return jdbcTemplate.query(sql.toString(), params.toArray(), // ✅ CORRECT
				(rs, rowNum) -> {
	...
	public List<CommissionDetailsDTO> fetchCommission(UUID userId, LocalDate fromDate, LocalDate toDate,
		...
		// ✅ Apply filter ONLY for non-common users
		if (!isCommon) {
			sql.append(" AND sc.agent_id = ?");
			params.add(userId);
		}

		if (fromDate != null) {
			sql.append(" AND sc.confirmed_at >= ? ");
			params.add(fromDate);
		}
		...
		sql.append(" ORDER BY sc.confirmed_at DESC ");
...
//                    dto.setIsReleased(rs.getBoolean("is_released"));
					dto.setConfirmedAt(
							rs.getDate("confirmed_at") != null ? rs.getDate("confirmed_at").toLocalDate() : null);
	...
	public UserPerformanceKpiDTO fetchKpis(List<UUID> userIds, UUID projectId, OffsetDateTime from, OffsetDateTime to) {
		...
		return jdbc.query(sql.toString(), params, rs -> {
			...
			return new UserPerformanceKpiDTO(rs.getInt("total_site_visits"), rs.getInt("total_sales"),
					rs.getBigDecimal("total_received"), rs.getBigDecimal("total_commission"));
		});
	}

	private MapSqlParameterSource baseParams(List<UUID> userIds, UUID projectId, OffsetDateTime from,
			OffsetDateTime to) {
		if (userIds == null || userIds.isEmpty()) {
			throw new IllegalArgumentException("userIds must not be empty");
		}
	...
	public PagedResponse<UserPerformanceSnapshotDTO> fetchSnapshotPage(List<UUID> userIds, UUID projectId,
		...
		List<UserPerformanceSnapshotDTO> rows = jdbc.query(dataSql, params, snapshotRowMapper());
