	...
	public List<SiteVisitSummaryDTO> fetchSiteVisitSummary(DashboardScope scope) {
	    ...
	    if (scope.isCustomer()) {
	        ...
	        if (scope.hasProjectScope()
	                && scope.getProjectIds() != null
	                && !scope.getProjectIds().isEmpty()) {
	    ...
	    return jdbc.query(
	        sql.toString(),
	        params,
	        new BeanPropertyRowMapper<>(SiteVisitSummaryDTO.class)
	    );
	}


	public List<SiteVisitSummaryDTO> fetchSiteVisitSummaryOld(DashboardScope scope) {
		StringBuilder sql = new StringBuilder("""
				    SELECT
				        sv.user_id          AS agent_id,
				        u.full_name         AS agent_name,
