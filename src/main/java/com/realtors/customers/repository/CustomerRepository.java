...
public class CustomerRepository {
	...
	private Object normalizeValue(String column, Object value) {
	    ...
	    // BIGINT columns
	    if (Set.of("mobile", "alt_mobile").contains(column)) {
	...
	public CustomerDto updatePartial(UUID customerId, Map<String, Object> updates) {
		...
		Set<String> disallowed = Set.of("customerId", "profileImage", "createdBy", "createdAt");
		...
		updates.forEach((key, value) -> {
			...
			Object normalized = normalizeValue(column, value);
		...
		try {
			...
			comments.sort(
				    Comparator.comparing(
				        c -> LocalDateTime.parse(c.get("addedAt").toString()),
