...
public class AppUtil {
	...
	public static BigDecimal percent(BigDecimal value) {
		return nz(value).divide(BigDecimal.valueOf(100), 6, RoundingMode.HALF_UP);
	}
	...
	public static void filterUserLookUp(DynamicFormResponseDto form, List<UserBasicDto> subordinates) {

		if (subordinates == null || subordinates.isEmpty()) {
			return;
		}

		Map<UUID, String> employeeIdByUserId = subordinates.stream()
				.collect(Collectors.toMap(UserBasicDto::userId, UserBasicDto::employeeId));

		Set<UUID> allowedUserIds = employeeIdByUserId.keySet();

		for (DynamicFormMetaRow row : form.getFields()) {

			if (!"manager_id".equals(row.getColumnName()) || !"select".equalsIgnoreCase(row.getFieldType())
			...
			List<Map<String, Object>> filtered = lookupData.stream().map(m -> {
				Object keyObj = m.get("key");
				if (keyObj == null)
					return null;

				try {
					UUID userId = keyObj instanceof UUID ? (UUID) keyObj : UUID.fromString(keyObj.toString());
	...
	public static void filterLookup(DynamicFormResponseDto form, List<UserBasicDto> subordinates,
		...
		for (DynamicFormMetaRow row : form.getFields()) {
			...
			if (subordinates != null && !subordinates.isEmpty()) {
				...
				if ("user_id".equals(row.getColumnName()) && "select".equalsIgnoreCase(row.getFieldType())
					...
					List<Map<String, Object>> filtered = lookupData.stream().filter(m -> {
						...
						try {
							UUID keyUuid = keyObj instanceof UUID ? (UUID) keyObj : UUID.fromString(keyObj.toString());
			...
			if (customers != null && !customers.isEmpty()) {
				Set<UUID> allowedCustomerIds = customers.stream().map(CustomerMiniDto::getCustomerId)
						.collect(Collectors.toSet());
				if ("customer_id".equals(row.getColumnName()) && "select".equalsIgnoreCase(row.getFieldType())
						&& row.getLookupData() != null) {
					List<Map<String, Object>> lookupData = (List<Map<String, Object>>) row.getLookupData();
					List<Map<String, Object>> filtered = lookupData.stream().filter(m -> {
						Object keyObj = m.get("key");
						return keyObj instanceof UUID && allowedCustomerIds.contains((UUID) keyObj);
					}).collect(Collectors.toList());
					row.setLookupData(filtered);
				}
			}
