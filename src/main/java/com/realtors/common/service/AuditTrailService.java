...
public class AuditTrailService {
	...
	public void auditAsync(String tableName, UUID recordId, EnumConstants action) {
		...
		auditAsyncInternal(tableName, recordId, action.toString(), performedBy, ipAddress, userAgent);
	}

	@Async("auditExecutor")
	public void auditAsyncInternal(String tableName, UUID recordId, String action, UUID performedBy, String ipAddress,
			String userAgent) {
		try {
			String sql = """
					    ...
					    VALUES (?, ?, ?, ?, ?, ?)
					""";
			jdbcTemplate.update(sql, tableName, recordId, action, performedBy, ipAddress, userAgent);
		} catch (Exception e) {
